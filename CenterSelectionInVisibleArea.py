class CenterSelectionInVisibleAreaCommand(sublime_plugin.TextCommand):
    def isEnabled(self, edit, args): return self.view.sel()
    def run(self, edit, args=None):
      ...
      for i in range(abs(diff)):
        if sel.begin() <= view.line(view.visible_region().begin()).begin() or sel.begin() >= view.line(view.visible_region().begin()).end():
          view.run_command("scroll_lines",  {"amount": (1 if diff >= 0 else -1)})
          sel = view.sel()[0]
        else:
          break
